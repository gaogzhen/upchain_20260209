...
contract C {
    ...
    function setDate(uint _data) internal {
        data = _data;
    }
...
contract CreateC {
    ...
    function create3Contract(
        uint _salt,
        bytes memory creationCode
    ) public returns (address) {
        return Create3.create3(keccak256(abi.encode(_salt)), creationCode);
    }
    ...
    function getCreateAddress(
        ...
        if (nonce > type(uint64).max - 1)
            revert InvalidNonceValue(address(this));
    ...
    function getCreate3Address(uint _salt) public view returns (address) {
        return Create3.addressOf(keccak256(abi.encode(_salt)));
    }
}
